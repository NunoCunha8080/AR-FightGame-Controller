...
class Switch():
    ...
    def setup(self, bbox_body):
        ...
        while True:
            ...
            if curr > TIMER_SETUP:
                break
            cv2.putText(frame, str(int(TIMER_SETUP - curr) + 1), POS_SCREEN, cv2.FONT_HERSHEY_SIMPLEX, 1.5, COLOR_RED,
                        4)
...
class Buttons:
    ...
    def buttons_bbox_init(self):  # to set up the regions of action (bbox of switch)
...
class Actions():
    ...
    def action_movement(self, pos):
        ...
        if abs(self.curr[0] - self.mid[
            0]) > self.delta_horizontal:  # move this from center, then steps will be recorded
            i = 1
            if delta_x > 0:
                i = 2
            for j in range(i):
